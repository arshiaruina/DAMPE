...
void LangauFit(string inFileName, string outFileName){
    ...
    for (int ibin=0; ibin<BGOenergyNbins; ibin++){  
        ...
        // FitHist(hist, frlo, frhi, lwidthlo, lwidthhi, lmpvlo, lmpvhi, area, gwidthlo, gwidthhi, flag);
        //TODO return type? 
        FitHist(hClustETot[ibin], 20, 120, 20, 60, 40, 80, 10e6, 0, 20, 0); // proton peak
        FitHist(hClustETot[ibin], 220, 380, 60, 100,230, 270, 10e6, 0, 20, 1); // helium peak
...
//TODO return type?
void FitHist(TH1D* hist, double frlo, double frhi, double lwidthlo, double lwidthhi, double lmpvlo, double lmpvhi, double area, double gwidthlo, double gwidthhi, int flag) {
    ...
    fr[0] = frlo;      fr[1] = frhi;
    pllo[0]=lwidthlo;              pllo[1]=lmpvlo;            pllo[2]=1.0;       pllo[3]=gwidthlo;
    plhi[0]=liwidthhi;             plhi[1]=lmpvhi;            plhi[2]=area;      plhi[3]=gwidthhi;
    sv[0]=(lwidthlo+liwidthhi)/2;  sv[1]=(lmpvlo+lmpvhi)/2;   sv[2]=area/2;      sv[3]=(gwidthlo+gwidthhi)/2;
    ...
    int ndf;
    //TODO function names?
    TF1 *fitFcnProton = langaufit(hist,fr0,sv0,pllo0,plhi0,fp0,fpe0,&chisqr0,&ndf0,flag);
...
void DrawHistWithFits(TH1D* hist, TF1* fitFcnProton, TF1* fitFcnHelium){    
