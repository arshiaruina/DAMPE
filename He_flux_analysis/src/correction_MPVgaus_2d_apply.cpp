...
int main(){
    ...
    // TODO: call the appropriate methods

    return 0;
} // end of main

void myLangauFit(std::string outFileName ){
    
    std::ifstream histFile;
    histFile.open("/beegfs/users/ruina/VAequalisation/resources/histolist_new.txt");
    std::string histName;
 
    //TODO histFile and histName will be defined inside the method
    //returns fp0[1] and fp1[1]
    //these will be input args for another function that does their histograms and computes corr facs
    ...
    //TODO how to pass 2d arrays as arguments
 
    bool corrFacApp;
    if(/* while computing the correction factors... */) //TODO need a flag for this 
        corrFacApp = true;
    else(/* while application of correction factors... */)
        corrFacApp = false; 
    //if before corr fac app
    makehMPV(MPV0[][], MPV1[][], corrFacApp, outfilename);

}//close myLangauFit()

//TODO outfile/outfilename has to be a common attribute shared between some of the methods
//so make them members of one class...

void makehMPV(mpv0Arr[][], mpv1Arr[][], flagPrePost, outfilename) {
...
void ComputeCorrFac() {
    ...
    ******************************************************/
    ...
    for(int iiladder = 0; iiladder < N_LADDER; iiladder++){
        for(int iiva = 0; iiva < N_VA; iiva++){
...
void ApplyCorrFac(){
    ...
    int nbins = hist0->GetXaxis()->GetNbins();
    TH1F *hnew = new TH1F("hnew","title",nbins,xminnew,xmaxnew);
    for (int i=1; i<=nbins; i++) { 
        double y = h->getBinContent(i); 
        double x = h->GetXaxis()->GetBinCenter(i); 
        double xnew = corrFac0[][] * x //correction 
        hnew->Fill(xnew,y); 
    }

} //close correctionFactors()
